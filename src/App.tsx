...
const ArticleIllustration: React.FC<{ icon?: string; title: string; category: string; className?: string; large?: boolean }> = ({ icon, title, category, className, large }) => {
  ...
  // Couleurs de fond basées sur le titre pour varier
  const colors = [
  ...
  return (
    <div className={cn("relative w-full h-full flex flex-col items-center justify-center overflow-hidden border border-zinc-100 transition-all duration-500", color, className)}>
      {/* Motif de fond type papier journal / grille */}
      ...
      {/* Éléments décoratifs type "dessin à la main" */}
      ...
      <div className="absolute bottom-4 w-full text-center">
        <p className="text-[7px] font-black uppercase tracking-[0.4em] text-zinc-400">ILLUSTRATION IA • L'ÉCHO</p>
      </div>
    </div>
  );
};
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          // Si pas de données, on vide les articles actuels pour forcer le message d'erreur/vide
          if (!cached) {
            setArticles([]);
            setError("Aucun article trouvé pour cette catégorie.");
          }
        }
      } catch (err) {
        console.error("Erreur de mise à jour:", err);
        setError("Impossible de charger les nouvelles. Vérifiez votre connexion ou votre clé API.");
      } finally {
        setLoading(false);
        setIsUpdating(false);
      }
        ...
        {loading && articles.length === 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12">
            <div className="md:col-span-12 lg:col-span-8 space-y-6">
              <div className="aspect-video skeleton rounded-sm flex items-center justify-center">
                <div className="text-center p-6">
                  <div className="w-16 h-16 border-4 border-zinc-200 border-t-zinc-900 rounded-full animate-spin mx-auto mb-6"></div>
                  <h3 className="font-serif text-2xl italic font-black mb-2">Recherche de la Vérité...</h3>
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">L'IA Atmani Bachir analyse le monde</p>
                </div>
              </div>
